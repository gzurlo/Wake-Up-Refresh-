...
const toCSV = (rows: any[]) => {
  const header = ["id","date","wakeTime","bedtime","snooze","energy","caffeineMg","hydrationPack","notes","refreshScore"];
  const body = rows.map(r => [r.id,r.date,r.wakeTime,r.bedtime ?? "",r.snooze,r.energy,r.caffeineMg,r.hydrationPack,r.notes ?? "",computeRefreshScore(r)]);
  return [header, ...body].map(arr => arr.map(v => typeof v === "string" && v.includes(",") ? `"${v.replaceAll('"','""')}"` : v).join(",")).join("\n");
};
...
const seedIfEmpty = () => {
  ...
  const seeded = Array.from({ length: 10 }).map((_, i) => {
    ...
    return { id: uid(), date: fmtDate(d), wakeTime: wake, bedtime: undefined, snooze: snooze, energy: energy, caffeineMg: caffeine, hydrationPack: hydration, notes: "" };
  });
...
export default function WakeUpRefreshMVP() {
  ...
  const nudgeTimer = useRef<number | null>(null); // Fixed: use number instead of NodeJS.Timeout
  ...
  useEffect(() => {
    ...
    nudgeTimer.current = window.setInterval(() => {
      ...
      const msg = msgs[Math.floor(Math.random()*msgs.length)]; // Fixed: Math.floor
  ...
  const getInsight1 = () => {
    ...
    const hydra = recent.filter((r: any) => r.hydrationPack);
    const noHydra = recent.filter((r: any) => !r.hydrationPack);
    const avgHydra = avg(hydra.map((r: any) => computeRefreshScore(r)));
    const avgNo = avg(noHydra.map((r: any) => computeRefreshScore(r)));
  ...
  const getInsight2 = () => {
    ...
    const withSnooze = recent.filter((r: any) => r.snooze > 0);
    const without = recent.filter((r: any) => r.snooze === 0);
    if(!withSnooze.length || !without.length) return "Aim for zero snoozes for one week to test impact.";
    const delta = (avg(without.map((r: any) => computeRefreshScore(r))) - avg(withSnooze.map((r: any) => computeRefreshScore(r)))).toFixed(1);
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-orange-50 text-slate-900">
      <header className="sticky top-0 z-10 backdrop-blur bg-gradient-to-r from-blue-950 to-orange-600 text-white border-b border-blue-900/40">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Wake‑Up <span className="text-orange-300">Refresh</span></h1>
          ...
          <div className="bg-white/90 backdrop-blur rounded-3xl border border-slate-200 shadow-lg p-5 sm:p-6">
            ...
            <ol className="space-y-2">
              {leaderboard.map((u, idx) => (
                <li key={u.name} className="flex items-center justify-between bg-gradient-to-r from-blue-50 to-orange-50 rounded-xl px-3 py-2 border">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold w-6 text-right text-blue-950">{idx+1}.</span>
                    <span>{u.emoji}</span>
                    <span className={u.name === "You" ? "font-bold text-orange-700" : "text-slate-800"}>
                      {u.name}
                    </span>
                  </div>
        ...
        <section className="xl:col-span-2 space-y-6">
          <div className="bg-white/90 backdrop-blur rounded-3xl border border-slate-200 shadow-lg p-5 sm:p-6">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <InsightCard title="Personalized Insight" desc={getInsight1()} />
              <InsightCard title="Snooze Behavior" desc={getInsight2()} />
            </div>
                ...
                <tbody>
                  {recent.map((r: any) => (
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="py-2 pr-3 max-w-[20ch] truncate" title={r.notes}>{r.notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <section className="xl:col-span-3">
          <div className="bg-gradient-to-r from-blue-900 to-orange-600 text-white rounded-3xl border border-blue-900/40 p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-lg">
            <div>
              ...
              <p className="text-white/90 text-sm">Start with a 2‑week sample (14 sticks). Log usage, then compare your Refresh Scores — data over hype.</p>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 mb-10">
        <div className="bg-white/90 backdrop-blur rounded-3xl border border-slate-200 shadow-lg p-6">
          ...
          <p className="text-sm text-slate-600 mb-3">Tune the app to your reality: where you struggle, what causes late nights, and whether you use a wearable.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <label>
              ...
              >
                {['Bird Library','Hinds Hall','Crouse Hall','Whitman','Maxwell','Life Sciences','Dome','Link Hall','Schine Center'].map(loc => 
            ...
            <fieldset className="sm:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                {['Late coding','Greek/social events','Design studio deadlines','Labs/clinicals','Work shifts','Gaming','Commuting','Sports practice'].map(tag => (
                  <button 
                    key={tag} 
                    type="button" 
                    onClick={() => setPilot(p => { 
                      const s = new Set(p.causes); 
                      s.has(tag) ? s.delete(tag) : s.add(tag); 
                      return {...p, causes: s}; 
                    })} 
                    className={`px-3 py-1 rounded-2xl border ${
                      pilot.causes.has(tag) ? 'bg-orange-600 text-white border-orange-600' : 'bg-white'
                    }`}
                  >
                    {tag}
                  </button>
            ...
            <div className="sm:col-span-2 flex justify-end">
              <button onClick={savePilot} className="px-4 py-2 rounded-2xl bg-orange-600 hover:bg-orange-700 text-white">
                Save Preferences
              </button>
            </div>
          </div>
      ...
      {showPilotSurvey && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-xl max-w-xl w-full p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">Help tailor nudges and campus challenges. (Saves locally in this demo.)</p>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="col-span-2 flex justify-end gap-2 mt-2">
                <button onClick={() => setShowPilotSurvey(false)} className="px-4 py-2 rounded-2xl border">Cancel</button>
                <button onClick={savePilot} className="px-4 py-2 rounded-2xl bg-orange-600 hover:bg-orange-700 text-white">Save</button>
              </div>
            </div>
          </div>
        </div>
